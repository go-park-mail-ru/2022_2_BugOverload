...
run-tests:
	go test -race ./internal/... -cover -coverpkg ./internal/...

get-stat-coverage:
	go test -race -coverpkg=./internal/... -coverprofile=c.out ./internal/...
	type c.out | findstr -v "easyjson" | findstr -v "mock" | findstr -v "dev" | findstr -v "test.go" | findstr -v "docs" |  findstr -v "testing.go" | findstr -v ".pb.go" | findstr -v "config" > c2.out
