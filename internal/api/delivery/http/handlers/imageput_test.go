...
func TestPutImageHandler_Action_OK(t *testing.T) {
	...
	// Check code
	require.Equal(t, http.StatusNoContent, w.Code, "Wrong StatusCode", pkg.GetResponseBody(*w))
}
...
func TestPutImageHandler_Action_NotOK(t *testing.T) {
	...
	err = easyjson.Unmarshal(bodyResp, &actualBody)
...
func TestPutImageHandler_Action_UserNotFound(t *testing.T) {
	// Init mock
	t.Parallel()
...
func TestPutImageHandler_Action_NotAdminIncorrectParams(t *testing.T) {
	// Init mock
	t.Parallel()
	...
	r := httptest.NewRequest(http.MethodPut, "/api/v1/image?object=INVALIDOBJECT&key=10", body)
