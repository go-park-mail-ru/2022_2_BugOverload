...
func (ad *AuthDatabase) GetUser(ctx context.Context, user *models.User) (models.User, error) {
	...
	err := sqltools.RunTxOnConn(ctx, innerPKG.TxDefaultOptions, ad.database.Connection, func(ctx context.Context, tx *sql.Tx) error {
		rowUser := tx.QueryRowContext(ctx, getUserByEmail, user.Email)
		...
		rowProfile := tx.QueryRowContext(ctx, getProfileAvatar, userDB.userID)
